function [NAMES_aux_sorted, REGION_aux_sorted, CIs,CIs_sum, post_mean_hat_sorted] = bootstrap_master_counter(nboot,NAMES_aux,REGION_aux,Y_aux,X_aux,Z_aux,RESULTS,RESULTS_boot,confidence_level)   
%% bootstrap_master_counter:  This function creates counterfactuals for subsets of agencies using bootstrapped coefficients alternative unobserved covariates (calculated using a boostrap sample of parameters).
%
% Author: José Luis Montiel Olea. Last Revised: June 4th, 2021
%
% Built using MATLAB Version: 9.10.0.1538726 (R2021a) Prerelease
%
% Syntax: function [NAMES_aux_sorted, REGION_aux_sorted, CIs,CIs_sum, post_mean_hat_sorted] = bootstrap_master_counter(nboot,NAMES_aux,REGION_aux,Y_aux,X_aux,Z_aux,RESULTS,RESULTS_boot,confidence_level)
%
% INPUT:
...
COUNTERS_only_alpha = zeros(J_LEAs,J_LEAs); 
%(i,j entry: covariates of agency i, alpha of agency j)

for i_LEA = 1:J_LEAs            
    for j_LEA = 1: J_LEAs
        
        COUNTERS_only_alpha(i_LEA,j_LEA) ...
...
TOTAL_FE = sum(COUNTERS_only_alpha)'; 

[~,I] = sort(TOTAL_FE,'descend');

NAMES_aux_sorted = NAMES_aux(I);

COUNTERS_sorted = COUNTERS_only_alpha(I,I); 
                           
post_mean_hat_sorted = post_mean_hat(I);
...
for i_boot = 1:nboot
    ...
    for i_LEA = 1:J_LEAs
        for j_LEA = 1:J_LEAs       
            COUNTERS_boots(i_LEA,j_LEA,i_boot) = counterfactual_function(post_mean_hat(j_LEA),X_aux_sorted(i_LEA,:,:),Z_aux_sorted(i_LEA,:),betas_i, gammas_i)-(i_LEA==j_LEA);                                   
        end
    end           
end
...
lowerCI_sum = reshape(quantile(sum(COUNTERS_boots,1),alpha/2,3),...
          [1,size(NAMES_aux_sorted,1)]);
      
upperCI_sum = reshape(quantile(sum(COUNTERS_boots,1),1-(alpha/2),3),...
          [1,size(NAMES_aux_sorted,1)]);  
