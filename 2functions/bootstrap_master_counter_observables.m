...
%% bootstrap_master_counter_observables:  This function creates counterfactuals for subsets of agencies using alternative observable covariates (calculated using a bootstrap sample of parameters). 
%
...
for i_boot = 1:nboot
    ...
    for i_LEA = 1:J_LEAs
        for j_LEA = 1:J_LEAs   
            ...
            COUNTERS_boots(i_LEA,j_LEA,i_boot) = counterfactual_function(post_mean_hat(i_LEA),X_aux_sorted(i_LEA,:,:),Z_aux_policy_sorted(:,:),betas_i,gammas_i)-(i_LEA==j_LEA);
        end
    end           
end
