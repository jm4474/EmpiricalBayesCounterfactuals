...
COUNTERS_diff = zeros(J_LEAs,J_LEAs);
...
for i_LEA = 1:J_LEAs            
    for j_LEA = 1: J_LEAs
       ...
       COUNTERS_diff(i_LEA,j_LEA) = COUNTERS_only_alpha(i_LEA,j_LEA) - COUNTERS_only_covs(i_LEA,j_LEA)
    end
end
...
for i_boot = 1:nboot
    ...
    for i_LEA = 1:J_LEAs
        for j_LEA = 1:J_LEAs   
            ...
            COUNTERS_boots(i_LEA,j_LEA,i_boot) = (counterfactual_function(post_mean_hat(j_LEA),X_aux_sorted(i_LEA,:,:),Z_aux_sorted(i_LEA,:),betas_i, gammas_i)-(i_LEA==j_LEA)) - (counterfactual_function(post_mean_hat(i_LEA),X_aux_sorted(j_LEA,:,:),Z_aux_policy_sorted(:,:),betas_i,gammas_i)-(i_LEA==j_LEA));
