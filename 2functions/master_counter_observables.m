...
%% master_counter_observables:  This function creates counterfactuals for subsets of agencies using alternative observed covariates. 
%
...
for i_boots = 1:size(beta_boots,1)
    ...
    for i_LEA = 1:J_LEAs
        ...
        COUNTERS_boots(i_LEA,j_LEA,i_boots) ...
                 = counterfactual_function(post_mean_hat_aux(i_LEA),...
                                        X_aux_sorted(i_LEA,:,:),...
                                        Z_aux_policy_sorted(:,:),...
                                        beta_aux,...
...
%% 7) Naive counterfactuals 

NAIVE_COUNTER ...
    = exp(Z_aux_sorted(:,2))*(sum(Y_aux_sorted,2)./exp(Z_aux_sorted(:,2)))';
