...
%          CIs,...
%          CIs_sum,...
%          NAIVE_counter] = bootstrap_master_counter_unobs_obs(nboot,NAMES_aux,REGION_aux,Y_aux,X_aux,Z_aux,RESULTS,confidence_level)
%
% INPUT:
...
for i_boot = 1:nboot
    ...
    for i_LEA = 1:J_LEAs
        for j_LEA = 1:J_LEAs   
            ...
            COUNTERS_boots(i_LEA,j_LEA,i_boot) = counterfactual_function(post_mean_hat(j_LEA),X_aux_sorted(i_LEA,:,:),Z_aux_policy_sorted(:,:),betas_i,gammas_i)-(i_LEA==j_LEA);
        end
    end           
end
